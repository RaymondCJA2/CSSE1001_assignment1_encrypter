...
def function_a():# ASK IF THIS NEEDS TO IDENTIFY #CHANGE IT TO TAKE SETS INSTEAD OF LIST ####cant handle aporsphties
    ...
    for encryption_offset_e in range(1, 26): # use a dictionary for this #replace "placeholder" with "non_alphabet"
        ...
        for placeholder in sentence:
            if placeholder in alphabet:      
                deciphered_text += alphabet[(alphabet.index(placeholder) - encryption_offset_e) % (len(alphabet))]
                deciphered_text_2 += alphabet[(alphabet.index(placeholder) - encryption_offset_e) % (len(alphabet))]

            elif placeholder ==" ":
                deciphered_text += placeholder#char
                deciphered_text_2 += placeholder

            else:
                deciphered_text += " " #deeper below you need to replace this space back with the weird symbol they gave
                deciphered_text_2 += placeholder
    ...
    for aa in range(1,26):
        keywords_to_look_for = DT_list[aa].split() 
        numbers_of_elements_to_loop_thru = len(keywords_to_look_for)

        pCounter = 0
        
        for string_ORSOMETHING in keywords_to_look_for: ##this is where the issue lies 
            if string_ORSOMETHING in words_list:
                ...
                break
    ##guess im doing it here then -> this is a slippery slope to drop into ngl
    #alphabet_space = ("abcdefghijklmnopqrstuvwxyz ")
    #for item in deciphered_text_2:
    #    if item !in alphabet_space:
    #        #get the index of that item(char) and put it in the same index(location) of deciphered_text_1
    for cc in range(1,len(DT_list)):
        for item in DT_list[cc]:
            if item == " ":
                #mark and remember its location so you can swap it with the special symbol from dt2l later
                

    deciphered_text_2_list = deciphered_text_2.split()
