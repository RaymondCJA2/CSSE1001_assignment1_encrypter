...
#IGNORE CONTRACTIONS -> '
## actually for ^^ just ask what is to be ignored and what is to be treated as a space other than - and '
#none of these functions can handle capital letters/new lines 
...
def function_e():
    ...
    if encryption_offset_e == 0:
        ...
        for encryption_offset_e in range(1, 26): # use a dictionary for this #replace "placeholder" with "non_alphabet"
            ...
            DT_list.insert(0,deciphered_text)

        for item in DT_list:
            print(str(DT_list.index(item)+1) + ": " + item)
    else:###        
...
def function_a():# ASK IF THIS NEEDS TO IDENTIFY #CHANGE IT TO TAKE SETS INSTEAD OF LIST ####cant handle aporsphties
    ...
    for aa in range(1,26):
        ...
        for char_ in DT_list[aa]:
            if char_ in alphabet:
                deci_text_2 += char_
            elif char_ == "'":
                pass
            else:
                deci_text_2 += " "
        DT_listb.append(deci_text_2)
                

        
        #DT_listb.append(re.sub("[^0-9a-zA-Z]+", " ", DT_list[aa])) #include ' in 0-9a-z
